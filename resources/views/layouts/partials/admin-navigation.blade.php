        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="shrink-0 flex items-center">
                    <a href="{{ route('dashboard') }}"> <!-- Ou route('admin.dashboard') se existir -->
                        <x-application-logo class="block h-9 w-auto fill-current text-gray-800 dark:text-gray-200" />
                    </a>
                    <span class="ml-2 text-gray-800 dark:text-gray-200 font-semibold">Admin Panel</span>
                </div>

                <!-- Navigation Links -->
                <div class="hidden space-x-8 sm:-my-px sm:ml-10 sm:flex">
                    {{-- Se tiver um dashboard de admin específico:
                    <x-nav-link :href="route('admin.dashboard')" :active="request()->routeIs('admin.dashboard')">
                        {{ __('Admin Dashboard') }}
                    </x-nav-link>
                    --}}
                     <x-nav-link :href="route('dashboard')" :active="request()->routeIs('dashboard')">
                        {{ __('Dashboard Utilizador') }} {{-- Link para o dashboard normal --}}
                    </x-nav-link>
                    <x-nav-link :href="route('admin.valves.index')" :active="request()->routeIs('admin.valves.*')">
                        {{ __('Válvulas') }}
                    </x-nav-link>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            <x-responsive-nav-link :href="route('dashboard')" :active="request()->routeIs('dashboard')">
                {{ __('Dashboard Utilizador') }}
            </x-responsive-nav-link>
            <x-responsive-nav-link :href="route('admin.valves.index')" :active="request()->routeIs('admin.valves.*')">
                {{ __('Válvulas') }}
            </x-responsive-nav-link>
